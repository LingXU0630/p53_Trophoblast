...
dnbc4tools rna mkref --ingtf /home/lingxu/DNBelab_C_Series_HT_scRNA-analysis-software/gencode.v32.primary_assembly.annotation.genes.filter.gtf --fasta /home/lingxu/DNBelab_C_Series_HT_scRNA-analysis-software/GRCh38.primary_assembly.genome.fa --thread 10  --species Homo_sapiens 

dnbc4tools rna run \
	--cDNAfastq1 /biomedja01/disk1/lxu/viola/sc-d27/WT_D2/V350183467_L01_15_1.fq.gz,/biomedja01/disk1/lxu/viola/sc-d27/WT_D2/V350183467_L02_15_1.fq.gz,/biomedja01/disk1/lxu/viola/sc-d27/WT_D2/V350183467_L03_15_1.fq.gz,/biomedja01/disk1/lxu/viola/sc-d27/WT_D2/V350183467_L04_15_1.fq.gz \
	--cDNAfastq2 /biomedja01/disk1/lxu/viola/sc-d27/WT_D2/V350183467_L01_15_2.fq.gz,/biomedja01/disk1/lxu/viola/sc-d27/WT_D2/V350183467_L02_15_2.fq.gz,/biomedja01/disk1/lxu/viola/sc-d27/WT_D2/V350183467_L03_15_2.fq.gz,/biomedja01/disk1/lxu/viola/sc-d27/WT_D2/V350183467_L04_15_2.fq.gz \
	--oligofastq1 /biomedja01/disk1/lxu/viola/sc-d27/WT_D2/V350183467_L01_11_1.fq.gz,/biomedja01/disk1/lxu/viola/sc-d27/WT_D2/V350183467_L02_11_1.fq.gz,/biomedja01/disk1/lxu/viola/sc-d27/WT_D2/V350183467_L03_11_1.fq.gz,/biomedja01/disk1/lxu/viola/sc-d27/WT_D2/V350183467_L04_11_1.fq.gz \
	--oligofastq2 /biomedja01/disk1/lxu/viola/sc-d27/WT_D2/V350183467_L01_11_2.fq.gz,/biomedja01/disk1/lxu/viola/sc-d27/WT_D2/V350183467_L02_11_2.fq.gz,/biomedja01/disk1/lxu/viola/sc-d27/WT_D2/V350183467_L03_11_2.fq.gz,/biomedja01/disk1/lxu/viola/sc-d27/WT_D2/V350183467_L04_11_2.fq.gz \
	--genomeDir /biomedja01/disk1/lxu/viola/sc-d27/star_dir  \
...
*********************2. remove unrelated cells and FindConservedMarkers

combined.sct.5<-subset(combined.sct,idents=c(0,1,2,3,4,6))
Idents(combined.sct.5)="orig.ident"
EPSC.5 <-subset(combined.sct.5,idents="EPSC")
EPSCKO.5 <-subset(combined.sct.5,idents="EPSCKO")
d2.5 <-subset(combined.sct.5,idents="D2")
kd2.5 <-subset(combined.sct.5,idents="KD2")
d7s.5 <-subset(combined.sct.5,idents="D7")
kd7s.5 <-subset(combined.sct.5,idents="KD7")
TSC.5 <-subset(combined.sct.5,idents="TSC")
TSCKO.5 <-subset(combined.sct.5,idents="TSCKO")

seurat_list_wt.5 <-list(EPSC.5,d2.5,d7s.5,TSC.5)
names(seurat_list_wt.5)<- c("EPSC","d2","d7s","TSC")

merged_seurat_wt.5 <- merge(x = seurat_list_wt.5[[1]],
		       y = seurat_list_wt.5[2:length(seurat_list_wt.5)],
		       merge.data = TRUE)

seurat_list_ko.5 <-list(EPSCKO.5,kd2.5,kd7s.5,TSCKO.5)
names(seurat_list_ko.5)<- c("EPSCKO","kd2","kd7s","TSCKO")

merged_seurat_ko.5 <- merge(x = seurat_list_ko.5[[1]],
		       y = seurat_list_ko.5[2:length(seurat_list_ko.5)],
		       merge.data = TRUE)

seurat_list <-list(merged_seurat_wt.5,merged_seurat_ko.5)
...
for (i in unique(Idents(combined.sct))) {
   ...
   write.csv(markers,paste0("combined.sct2000_CR5.TSC_conserve_markers_cluster",i,"csv"))  
  }
...
wt<-subset(combined.sct,idents="WT")
Idents(wt) <-"xlcluster"
wt.sling<- subset(combined.sct,idents=c("C5-TSC","C6-TSC","C7-TSC")) 

cds <- as.cell_data_set(wt.sling)
...
ko.sling<- subset(combined.sct,idents=c("TSCKO")) 
cell_to_remove <-c("TSCKO_Cell3393_CELL72957_N1","TSCKO_Cell1825_CELL23031_N1","TSCKO_Cell1691_CELL18458_N1","TSCKO_Cell679_CELL4196_N2")# coming from day7
ko.sling<- subset(ko.sling,cells = cell_to_remove,invert=TRUE)
...
wt.sling<- subset(combined.sct,idents=c("TSC")) 
cell_to_remove <-c("TSCKO_Cell3393_CELL72957_N1","TSCKO_Cell1825_CELL23031_N1","TSCKO_Cell1691_CELL18458_N1","TSCKO_Cell679_CELL4196_N2")# coming from day7
wt.sling<- subset(wt.sling,cells = cell_to_remove,invert=TRUE)
...
sling<- subset(combined.sct,idents=c("TSCKO","TSC")) 
cell_to_remove <-c("TSCKO_Cell3393_CELL72957_N1","TSCKO_Cell1825_CELL23031_N1","TSCKO_Cell1691_CELL18458_N1","TSCKO_Cell679_CELL4196_N2")# coming from day7
wt.sling<- subset(sling,cells = cell_to_remove,invert=TRUE)
...
umap = pd.read_csv("/biomedja01/disk1/lxu/viola/scRNAseq-TSC/scEPSC/scVelo/TSC_loomOrder_embedding_obs.csv")
clusters = pd.read_csv("/biomedja01/disk1/lxu/viola/scRNAseq-TSC/scEPSC/scVelo/TSC_loomOrder_cluster_obs.csv")
...
***2.heatmap tp53 related

gene2<-c("TP53", "CDKN1A", "MDM2", "PHLDA3", "BAX", "BBC3", "CCNG1", "GDF15", "PURPL", "TNFRSF10B", "NEAT1", "DDB2", "ZMAT3", "PTPN14")
DefaultAssay(wt)<-"RNA"
tp53<-wt@assays$RNA[gene2]
tp53<- data.frame(tp53)
id<-as.character(wt@meta.data$xlcluster)
names(id)<-colnames(tp53)
tp53<- tp53[,match(names(sort(id)),colnames(tp53))]
tp53=as.matrix(tp53)
tp53<-t(scale(t(tp53), center = TRUE, scale = TRUE))

col_fun = colorRamp2(c(-1,1), c("white","#4569bb"))
id<-sort(id)
row_ha = HeatmapAnnotation(bar =id ,show_legend = c("bar" = TRUE),col = list(bar = c("C0-EPSC" = "#4583b3", "C1-D2" = "#51c4c2","C2-D2"="#0d8a8c","C3-D7"="#be86ba","C4-D7"="#f2db96","C5-TSC"="#f172ad","C6-TSC"="#f7afb9","C7-TSC"="#c63596")))
pdf("./manuscript_plot/heatmap_tp53_combined.sct_scale-wt2.pdf", width = 8, height =4)
Heatmap(tp53, name="na",cluster_columns = FALSE,col = col_fun,top_annotation = row_ha,cluster_rows = FALSE,use_raster=FALSE,row_names_side = "left",row_names_gp=gpar(fontsize = 6),show_column_names = FALSE,show_row_dend=FALSE)
dev.off()
...
***5.heatmap plot of clusters DEGs
